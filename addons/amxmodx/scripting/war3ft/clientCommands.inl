...
CMD_Handle( id, szCmd[], bool:bThroughSay )
{
	...
	if ( CMD_Equal( id,  szCmd, "changerace" ) || CMD_Equal( id, szCmd, "race" ) )
	{
		WC3_ChangeRaceStart( id );
	}
	
	// Display select skill menu
	else if ( CMD_Equal( id,  szCmd, "selectskills" ) || CMD_Equal( id,  szCmd, "selectskill" ) || CMD_Equal( id, szCmd, "skills" ) || CMD_Equal( id, szCmd, "skill" ) )
	{
		MENU_SelectSkill( id );
	}
	...
	else if ( CMD_Equal( id,  szCmd, "war3menu" ) || CMD_Equal( id, szCmd, "menu" ) )
	{
		MENU_War3Menu( id );
	}
	...
	else if ( CMD_Equal( id,  szCmd, "resetskills" ) || CMD_Equal( id, szCmd, "reset" ) || CMD_Equal( id, szCmd, "reselect" ) )
	{
	...
	else if ( CMD_Equal( id, szCmd, "cloak" ) || CMD_Equal( id, szCmd, "invis" ) )
	{
	...
	else if ( CMD_Equal( id, szCmd, "necklace" ) || CMD_Equal( id, szCmd, "neck" ) || CMD_Equal( id, szCmd, "block" ) )
	{
		if (ITEM_MenuCanBuyCheck(id)) ITEM_Buy( id, ITEM_NECKLACE );
	}

	else if ( CMD_Equal( id, szCmd, "frost" ) || CMD_Equal( id, szCmd, "orb" ) )
	{
	...
	else if ( CMD_Equal( id, szCmd, "tome" ) || CMD_Equal( id, szCmd, "buyxp" ) )
	{
	...
	if ( get_pcvar_num( CVAR_wc3_races ) > 4 )
	{
		...
		else if ( CMD_Equal( id, szCmd, "scroll" ) || CMD_Equal( id, szCmd, "respawn" ) )
		{
			if (ITEM_MenuCanBuyCheck(id)) ITEM_Buy( id, ITEM_SCROLL );
		}

		else if ( CMD_Equal( id, szCmd, "helm" ) || CMD_Equal( id, szCmd, "hs" ) )
		{
			if (ITEM_MenuCanBuyCheck(id)) ITEM_Buy( id, ITEM_HELM );
		}

		else if ( CMD_Equal( id, szCmd, "amulet" ) || CMD_Equal( id, szCmd, "silence" ) )
		{
