...
public WC3_SetRandomBotLevel( id ) 
{
	new iBotStartLevel = get_pcvar_num( CVAR_wc3_start_level ); 
	new iBotLevel = random_num(iBotStartLevel,MAX_LEVELS);
	new iBotXP = XP_GetByLevel( iBotLevel );
	p_data[id][P_XP] = iBotXP;
...
WC3_ShowBar( id )
{
	...
	if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
	{
		...
		else
		{
			...
			{
				pos += formatex( szRaceInfo[pos], 255, "%s  %L: %d  ", szShortRaceName, id, "WORD_LEVEL", p_data[id][P_LEVEL], p_data[id][P_XP] );
...
WC3_ShowSpecInfo( id, iTargetID )
{
	...
	{
		iMsgPos += formatex( szMsg, 511, "%s  %L: %d", szRaceName, id, "WORD_LEVEL", p_data[iTargetID][P_LEVEL]);
	}
	...
	if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
	{
		iMsgPos += formatex( szMsg[iMsgPos], 512-iMsgPos, "^n%L: %d  %L: %d^n^n/race   /raceinfo   /shop   /shop2", id, "CURRENT_HEALTH", get_user_health( iTargetID ), id, "WORD_ARMOR", get_user_armor( iTargetID ) );
	}
