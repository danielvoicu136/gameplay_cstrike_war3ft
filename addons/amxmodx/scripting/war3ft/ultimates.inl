...
public _ULT_Delay()
{
	...
	for ( i = 0; i < numplayers; i++ )
	{
		...
		if(is_user_alive( id ) && (( ITEM_Has( id, ITEM_CLOAK ) > ITEM_NONE ) || ( iSkillLevelInvisibility > 0 ))) { 
		
			if (IsCurrentSpeedHigherThan(id, INVIS_SPEED_LIMIT)) {
		...
		// Shadow Hunter FIX --- try to activate healing 
		static iSkillLevelHealing;
		iSkillLevelHealing = SM_GetSkillLevel( id, SKILL_HEALINGWAVE );
		if(is_user_alive( id ) && ( iSkillLevelHealing > 0 ) ) { 
			SH_HealingWave( id )
		}
