...
public SHARED_INVIS_Set( id )
{
	...
	if ( ITEM_Has( id, ITEM_CLOAK ) > ITEM_NONE )
	{
		...
		{
			iInvisLevel = p_invisibility[MAX_SKILL_LEVEL-1];
		}
	}
	...
	{
		SHARED_INVIS_ReSet( id );
	}

	return;
}

// This will reset the invisibility 
public SHARED_INVIS_ReSet( id ) { 
...
public SHARED_SetBonusReward(id) { 
    if (p_data[id][P_LEVEL] < MAX_LEVELS) { 
        ...
        client_print(id, print_chat, "%s You earned %d XP for playing", g_MODclient, iBonusXP);
    } 
    else { 
        
        new Float:fXPMult = get_pcvar_float(CVAR_wc3_xp_multiplier);
        new iMaxXP = 500 + floatround(iXPLevelSaved[MAX_LEVELS] * fXPMult); 
        new iCurrentXP = p_data[id][P_XP];

        if (iCurrentXP > iMaxXP) {
            new iCurrentMoney = cs_get_user_money(id);

          
            new iExcessXP = iCurrentXP - iMaxXP;
            new iMoneyDeficit = MAX_MONEY - iCurrentMoney;

           
            new iXPToConvert = 0;
            if (iExcessXP <= iMoneyDeficit) {
                iXPToConvert = iExcessXP;
            } 
            else {
                iXPToConvert = iMoneyDeficit;
            }

            if (iXPToConvert > 0) {
                p_data[id][P_XP] -= iXPToConvert;

                if (p_data[id][P_XP] < iMaxXP) {
                    p_data[id][P_XP] = iMaxXP;
                }

                cs_set_user_money(id, iCurrentMoney + iXPToConvert);

                client_print(id, print_chat, "%s Excess %d XP was converted to %d $", g_MODclient, iXPToConvert, iXPToConvert);
            }
        }
    }
}
