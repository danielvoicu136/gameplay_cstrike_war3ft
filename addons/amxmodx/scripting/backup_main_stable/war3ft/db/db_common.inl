...
DB_GetKey(id, szKey[], len)
{
	...
	case DB_SAVEBY_IP:
		get_user_ip(id, szKey, len, 1);
	case DB_SAVEBY_STEAMID:
		get_user_authid(id, szKey, len);
	}
}
...
DB_GetKeyName(szKeyName[], len)
{
	...
	{
	case DB_SAVEBY_NAME:
		copy(szKeyName, len, "player_name");
	case DB_SAVEBY_IP:
		copy(szKeyName, len, "player_ip");
	case DB_SAVEBY_STEAMID:
		copy(szKeyName, len, "player_steamid");
	}
}
...
bool:DB_Connection_Available()
{
	...
	{
	case DB_MYSQLX:
		return MYSQLX_Connection_Available();
	case DB_SQLITE:
		return SQLITE_Connection_Available();
	}
